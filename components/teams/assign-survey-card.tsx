...
export function AssignSurveyCard({
  ...
  React.useEffect(() => {
    setSelectedTemplateId(templates[0]?.id ? String(templates[0].id) : undefined);
  }, [templates]);
      ...
      <CardContent className="p-4 space-y-3">
        {templates.length > 0 ? (
          ...
          <div className="rounded-lg border border-dashed border-muted-foreground/30 bg-muted/10 px-3 py-2 text-sm text-muted-foreground">
            В базе нет шаблонов опросов. Нажмите «Создать шаблоны», чтобы добавить
            базовые (PSS-14 и Gallup Q12).
          </div>
        )}
        ...
        <div className="flex flex-col sm:flex-row gap-2">
          <Button
            ...
            disabled={isPending || !selectedTemplateId || templates.length === 0}
