...
export function ForgotForm({ className, ...props }: React.ComponentProps<"form">) {
    ...
    if (state?.success) {
        return (
            <div className="flex flex-col items-center gap-4 text-center max-w-sm mx-auto p-8 bg-background rounded-xl border shadow-sm">
        ...
        <form action={formAction} className={cn("flex flex-col gap-6 w-full max-w-sm mx-auto", className)} {...props}>
            <FieldGroup>
                ...
                <Field>
                    ...
                    <Input id="email" type="email" name="email" placeholder="m@example.com" required className="bg-background" />
                </Field>
                ...
                <div className="text-center">
                    <a href="/login" className="inline-flex items-center gap-2 text-sm text-muted-foreground hover:text-primary transition-colors">
